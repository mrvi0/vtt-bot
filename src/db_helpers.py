...
async def get_db_connection(): # Функция стала асинхронной
    db_path = config.DB_NAME
    os.makedirs(os.path.dirname(db_path), exist_ok=True)
    return await aiosqlite.connect(db_path) # Используем aiosqlite.connect

async def init_db(): # Асинхронная
    try:
        async with await get_db_connection() as db:
            await db.execute('''CREATE TABLE IF NOT EXISTS stats
                                  (id INTEGER PRIMARY KEY AUTOINCREMENT,
                                   ...
                                   message_type TEXT, 
                                   date DATE)''')
            # Новая таблица для счетчиков рекламы по чатам
            await db.execute('''CREATE TABLE IF NOT EXISTS chat_ad_counters
...
async def record_stat(user_id: int, message_type: str = "voice"): # Асинхронная
    try:
        async with await get_db_connection() as db:
        ...
        logger.error(f"SQLite error in record_stat: {e}", exc_info=True)

async def get_stats(): # Асинхронная
...
# --- Новые функции для управления счетчиком рекламы ---
async def increment_chat_ad_counter(chat_id: int) -> int:
    """Увеличивает счетчик сообщений для чата и возвращает новое значение."""
    new_count = 0
    try:
        async with await get_db_connection() as db:
...
async def reset_chat_ad_counter(chat_id: int):
    """Сбрасывает счетчик сообщений для чата на 0."""
    try:
        async with await get_db_connection() as db:
